classdef helper
    ...
    methods(Static)
        ...
        function my_plot(time, traj, x_s, y_s, theta_s, S_p_x, S_p_y, u_n, u_t,cost_function_vect, mode_vect)
            ...
            xlim([ax1,ax2,ax3,ax4,ax5,ax6,ax7,ax8,ax9],[0 time(end)])
        end
        ...
        function [x_s, y_s, theta_s, S_p_x, S_p_y, u_n, u_t, time_sim_vec,mode_vect] = closed_loop_matlab(plant, controller,x0, time_sim, print_, sim_noise, debug_cost)
            ...
            for i = 1:time_sim_
                ...
                if i > time_sim_/3 && i < (time_sim_/3+1)
                    disp("Disturbance")
                    x(2,i) = x(2,i) - 0.02;
                end
                ...
                if debug_cost == true
                    if(i*controller.sample_time > 0.3 && i*controller.sample_time < 0.9 && mod(i,2)==0)
                        index = index + 1;
...
%                         u(:,i) = u_min;
        ...
        function u_min = debug_cost_function(x0, u_n_lb, u_t_lb, u_n_ub, u_t_ub, controller, plant, index, t)
            ...
            if index <10
                ...
                u_min = [X(ind_raw(1),ind_col); Y(ind_raw(1),ind_col)];
                xlabel('u_n [m/s]')
                ylabel('u_t [m/s]')
